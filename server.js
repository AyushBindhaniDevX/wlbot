...
async function startApp() {
  ...
  console.log("John's Tech Licensing System - ONLINE - " + a.toString())
}
startApp();
...
client.on("ready", () => {
  client.user.setActivity("people use ;help!", { 
    type:3,
    browser: "dnd"
  })
})
    ...
    if (!product) {
      ...
      client.guilds.resolve(process.env.PRIMARYGUILD).members.fetch(person.discord).then(userrr => {
        ...
        const embed = new Discord.MessageEmbed()
          ...
          .setDescription("You have now been granted a license for `" + product.name + "`. We hope you enjoy your purchase! Please direct message John5287#5287 for any issues with whitelisting system!");
        userrr.send(embed)
        var response = {
          "status": "success"
        }
